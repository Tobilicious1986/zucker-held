# Zucker-Held — Projektregeln für KI-Agenten
...
## PFLICHTREGELN — Codex, Claude und andere KI-Tools müssen diese immer einhalten

Diese Datei ist die zentrale ausführliche Regelquelle. Die Root-Dateien `../../AGENTS.md` und `../../CLAUDE.md` bleiben absichtlich kurz und verweisen hierher, damit Tool-Autodiscovery zuverlässig funktioniert.

### Branching & Git-Disziplin
- Die verbindlichen Branch-Regeln stehen in `docs/agents/BRANCHING.md`.
- Die agentenübergreifende Sprint- und Kollaborationslogik steht in `docs/agents/AGENT_WORKFLOW.md`.
- Bei jeder Git-Arbeit muss `docs/agents/BRANCHING.md` vor Branch-, Merge- oder Push-Entscheidungen beachtet werden.
- Bei Konflikt zwischen Ad-hoc-Workflow und `docs/agents/BRANCHING.md` gilt `docs/agents/BRANCHING.md`.
...
- Mindestens zu prüfen sind dabei: `README.md`, `docs/project/ARCHITECTURE.md`, `docs/project/COOKBOOK.md`, `docs/project/BACKLOG.md` sowie sprintbezogene Review-/UAT-Dokumente unter `docs/reviews/` und `docs/uat/`.
- Für jeden aktiven Sprint muss zusätzlich eine fortlaufende Arbeitsdatei `docs/sprints/SPRINT{N}.md` gepflegt werden, damit bei Ausfällen oder Kontextverlust jederzeit klar ist, was bereits erledigt, in Arbeit oder als Nächstes geplant ist.
- `docs/sprints/SPRINT{N}.md` ist vom Sprint-Start bis zum Sprint-Abschluss laufend mitzupflegen und gehört zur Pflichtdokumentation des Sprints.
- Architekturänderungen, Integrationsänderungen, Rollen-/State-/API-Änderungen oder neue Querschnittsmuster dürfen nicht abgeschlossen werden, ohne dass `docs/project/ARCHITECTURE.md` auf den Ist-Zustand nachgezogen wird.
...
- Jeder parallele Strang bekommt einen klaren Owner; ein führender Agent bleibt immer für Planung, Priorisierung, Zusammenführung, Endabnahme und die Pflege von `docs/sprints/SPRINT{N}.md` verantwortlich.
...
- Während längerer Arbeiten müssen in regelmäßigen Abständen kurze Dailies/Synchronisationen stattfinden; offene Fragen, Risiken, Entscheidungen und nächste Schritte sind in `docs/sprints/SPRINT{N}.md` mitzuprotokollieren.
...
- `docs/sprints/SPRINT{N}.md` ist das gemeinsame Lagebild für Parallelisierung, Dailies, Cross-Reviews, offene Blocker und den letzten stabilen Stand.
...
- Jeder Zyklus enthält mindestens: kurze Zielschärfung, Umsetzung, Daily/Synchronisation, Challenge durch einen zweiten Agenten oder Spezialisten und einen Eintrag in `docs/sprints/SPRINT{N}.md`.
...
- Details zu Dailies, Cross-Reviews, agentenspezifischen Commits, Retros und Laufzeitpruefung stehen in `docs/agents/AGENT_WORKFLOW.md`.
...
- UAT-Szenarien (in `docs/uat/UAT_SPRINT_{N}.md` oder ähnlichen Dateien) dürfen nur dann mit ✅ versehen werden, wenn die Szenarien manuell oder automatisiert wirklich durchgespielt wurden.
...
- Wenn Teile eines Sprints nicht testbar sind (z. B. weil kein laufender Server verfügbar ist), muss das explizit in `docs/sprints/SPRINT{N}.md` vermerkt werden mit dem genauen Grund und dem Plan, wann und wie nachgeholt wird.
...
- Für Full-Stack-Arbeit gelten die tatsächlichen Stack-Regeln aus `docs/project/ARCHITECTURE.md`, `frontend/CLAUDE.md`, `frontend/AGENTS.md` und den vorhandenen `backend/`-/`frontend/`-Konventionen.
- Legacy-PWA:
...
│
├── AGENTS.md           — kurzer Root-Einstieg für Codex
├── CLAUDE.md           — kurzer Root-Einstieg für Claude
└── docs/               — ausführliche Agenten-, Projekt-, Sprint-, Review- und UAT-Doku
```
